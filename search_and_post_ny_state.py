...
def find_local_notables_state(bird_key):
    notables = get_notable_observations(bird_key, 'US-NY',back=1)
...
def write_nyc_text(df,url,headers):
    ...
    text = f"Notable bird! The {name}, also known as the {sci_name}. It was spotted at {locname}. An image of the {name} is below, if available. Here is a link to the eBird checklist: https://ebird.org/checklist/{cl_id}"
...
def main():
    ...
    parser.add_argument('count', help='Number of birds to post')

    args = parser.parse_args()
    count = int(args.count)
    print("Starting main process...")
    try:
        df = find_local_notables_state(bird_key)
        print("Found local notables")
        b_list, short_df = make_list(df,count)
        print(f"Made list of {count} birds: {b_list}")
